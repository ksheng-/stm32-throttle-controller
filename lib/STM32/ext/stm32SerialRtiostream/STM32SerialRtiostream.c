...
/* Set when current send is terminated */
...
int rtIOStreamOpen(int argc, void * argv[])
{
	...
	//Start Receive
	 rcvOn = 1;
	HAL_UART_Receive_IT(EXT_MODE_USART_HANDLE, (uint8_t*)ptSet, (uint16_t)1);
...
int rtIOStreamSend(int streamID, const void *src, size_t size, size_t *sizeSent)
{
	...
	*sizeSent = 0;
    ...
    if(HAL_OK == HAL_UART_Transmit_IT(EXT_MODE_USART_HANDLE, (uint8_t*)src, (uint16_t)size))
	{
		...
		//Wait for end of Tx
        while(!TxCplt && ((HAL_UART_GetState(EXT_MODE_USART_HANDLE) == HAL_UART_STATE_BUSY_TX) || (HAL_UART_GetState(EXT_MODE_USART_HANDLE) == HAL_UART_STATE_BUSY_TX_RX)));	
		if(TxCplt)
			*sizeSent = size;	
	} 
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	//Inc pt on receive buffer
	ptSet++;
	//Inc nb of rcv char
	nbRcv++;
	//Test end of rcv buffer
